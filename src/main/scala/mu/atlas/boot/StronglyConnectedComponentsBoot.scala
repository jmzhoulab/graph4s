...
import mu.atlas.graph.community.StronglyConnectedComponents
...
/**
  ...
  */
class StronglyConnectedComponentsBoot extends Executor {
  ...
  override def execute[N: ClassTag](edgeTuple: RDD[(N, N)], prop: Props = null): RDD[String] = {
    ...
    val ccVertex = StronglyConnectedComponents.run(graph).vertices
